...
abstract class BaseViewModel<S : UiState, I : Intent, SE : SideEffect>(
    ...
    // SideEffect
    private val sideEffectHolder = EffectEmitter<SE>(logger)
     ...
     * */
    protected suspend fun emitEffect(effect: SE) {
        sideEffectHolder.emit(effect)
    }

    protected fun tryEmitEffect(effect: SE) {
    ...
    protected fun <D> launchResult(
        ...
        onSuccess: suspend (D) -> Unit, // 비동기 메서드 호출이 성공했을 때 처리해야 할 로직
    ...
    protected open fun handleError(error: AppError) {
        when (error) {
            is AppError.Http ->
                logger.e {
                    "HTTP error: status=${error.status}, code=${error.errorCode}, message=${error.message}"
                }
